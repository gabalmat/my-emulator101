...
int dissasemble_8080( unsigned char *codebuffer, int pc)
{
     ...
     * STA  a   - Store A in memory address
     ...
     * POP  RP  - Pop from stack and store in RP. SP=SP+2
     * PUSH BP  - Push RP onto stack. SP=SP-2
     * XTHL     - Exchange H:L with top word on stack
     * PCHL     - Put H:L into PC (program counter). (=JMP HL)
     * XCHG     - Exchange H:L with D:E
     * SPHL     - Put H:L into SP
     *      8-bit arithmetic
     ...
     * ADD  S   - Add register S to A
     * ADC  S   - Add register S to A with carry
     ...
     * SBI      - Subtract immeditae from A with borrow
     *     16-bit arithmetic
     ...
     * RNC      - If no carry, RET
     * RC       - If carry, RET
     * RPO      - If parity odd, RET
     * RPE      - If parity even, RET
     * RP       - If plus, RET
     * RM       - If minus, RET
     * JMP a    - Unconditional jump to a. PC <- a
     * JNZ a    - If NZ, JMP a
     * JZ  a    - If Z, JMP a
     * JNC a    - If no carry, JMP a
     * JC  a    - If carry, JMP a
     * JPO a    - If parity odd, JMP a
     * JPE a    - If parity even, JMP a
     * JP  a    - If plus, JMP a
     * JM  a    - If minus, JMP a
     * CALL a   - Unconditional subroutine call to a
     * CZ  a    - If Z, CALL a
     * CNC a    - If no carry, CALL a
     * CC  a    - If carry, CALL a
     * CPO a    - If parity odd, CALL a
     * CPE a    - If parity even, CALL a
     * CP  a    - If plus, CALL a
     * CM  a    - If minus, CALL a
     *     Other
     * NOP      - No Op
     ...
     * IN  PORT - Write data from PORT to A
    ...
    switch (*code)
    {
        ...
        case 0xfe: printf("CM     #$%02x", code[1]); opbytes = 2; break;
